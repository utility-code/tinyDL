...
def info(func):
    ...
    def wrapper(*func_args, **func_kwargs):
        ...
        print(func.__name__+'() :: ' + repr(retval))
        return retval
    return wrapper
...
def default_stacker(batch):
    ...
    if isinstance(batch[0], (list, tuple)):
        return tuple(default_stacker(var) for var in zip(*batch))


class NaiveDataLoader:
    # No multiprocessing data loader
    def __init__(self, dataset, batch_size=64, collate_fn=default_stacker):
        self.index = 0
    ...
    def __next__(self):
        if self.index >= len(self.dataset):
            raise StopIteration
        batch_size = min(len(self.dataset) - self.index, self.batch_size) #what?
...
class DataLoader(NaiveDataLoader):
    def __init__(
        self, dataset, batch_size = 64, num_workers = 1, prefetch_batchs = 2, collate_fn = default_stacker
    ):
    ...
    def prefetch(self):
        while(
            self.prefetch_index <len(self.dataset) and self.prefetch_index < self.index +2 * self.num_workers * self.batch_size
        ):
